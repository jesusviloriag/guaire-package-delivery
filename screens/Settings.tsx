...
function Profile({ navigation }): JSX.Element {
  ...
  useEffect(() => {
    ...
    setUsername(GuairePackageDelivery.instance.user.username)
    setAvatar(GuairePackageDelivery.instance.user.avatar)
  }, []);
  ...
  const save = () => {
    GuairePackageDelivery.instance.saveUser({
      username: username,
      avatar: avatar
    });
    navigation.pop();
  ...
  const logout = () => {
    GuairePackageDelivery.instance.logout().then((result) => {
      if(result) {
